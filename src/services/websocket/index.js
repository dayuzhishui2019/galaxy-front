...
var SunsetWebsocket = {
	...
	_initEvent: function (ctx) {
		...
		connection.onopen = function () {
			...
			if (ctx.heart) {
				ctx.heartTimer = setInterval(() => {
					self.send(topic, Sunset.isFunction(ctx.heart.message) ? ctx.heart.message() : ctx.heart.message);
				}, ctx.heart.interval || 5000);
			}
			console.log("open");
		};
		connection.onmessage = function (evt) {
			var formatData = evt.data;
			try {
				if (format) {
		...
		connection.onclose = function (evt) {
			if (connection && !ctx.once) {
				console.log("断线重连");
				self._connect(ctx);
			} else {
				self.trigger(topic, EVENTS.DISCONNECT);
			}
			clearTimeout(ctx.heartTimer);
			console.log("close");
		};
	...
	listen: function (topic, eventName, cb) {
	...
	unlisten: function (topic, eventName, cb) {
		var pubsub = this.pubsubMap[topic];
		if (pubsub) {
...
module.exports = window.$websocket = window.$ws = SunsetWebsocket;